...
// Assign ke window supabase untuk kompatibilitas dengan kode yang masih menggunakan supabase.
window.supabase = sb;
...
// ========== SUPABASE HELPER FUNCTIONS (Firestore style) ==========
// Fungsi ini dibuat agar kode yang menggunakan Firestore bisa langsung bekerja dengan Supabase
const db = {
  collection: (name) => {
    return {
      doc: (id) => {
        return {
          ...
          set: async (data, options) => {
            const { error } = await sb.from(name).upsert({ ...data, id: id, updated_at: new Date().toISOString() });
            if (error) throw error;
          },
          update: async (data) => {
            const { error } = await sb.from(name).update({ ...data, updated_at: new Date().toISOString() }).eq('id', id);
            if (error) throw error;
          },
          delete: async () => {
      ...
      where: (field, operator, value) => {
        ...
        return {
          get: async () => {
            ...
            return { docs: (data || []).map(d => ({ id: d.id, data: () => d, exists: true })), size: data?.length || 0 };
          },
          onSnapshot: (callback) => {
      ...
      orderBy: (field, direction) => {
        let query = sb.from(name).select('*').order(field, { ascending: direction !== 'desc' });
        return {
          get: async () => {
            const { data, error } = await query;
            return { docs: (data || []).map(d => ({ id: d.id, data: () => d, exists: true })), size: data?.length || 0 };
          ...
          startAfter: (lastDoc) => {
            // Implementasi sederhana untuk startAfter
            return {
              limit: (limit) => {
                return {
                  get: async () => {
                    let newQuery = sb.from(name).select('*').order(field, { ascending: direction !== 'desc' });
                    if (lastDoc && lastDoc[field]) {
                      if (direction !== 'desc') {
                        newQuery = newQuery.gt(field, lastDoc[field]);
                      } else {
                        newQuery = newQuery.lt(field, lastDoc[field]);
                      }
                    }
      ...
      limit: (limit) => {
        let query = sb.from(name).select('*').limit(limit);
        return {
          get: async () => {
            const { data, error } = await query;
            return { docs: (data || []).map(d => ({ id: d.id, data: () => d, exists: true })), size: data?.length || 0 };
      ...
      add: async (data) => {
        const { data: result, error } = await sb.from(name).insert({ ...data, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select();
        if (error) throw error;
    ...
    return {
      commit: async () => {
        for (const op of operations) {
          if (op.type === 'delete') {
            await sb.from(op.collection).delete().eq('id', op.id);
          }
        }
      },
      delete: (docRef) => {
        operations.push({ type: 'delete', collection: docRef._collection, id: docRef._id });
      }
    };
  },
  settings: (options) => {
    // No-op for Supabase compatibility
    console.log('Firestore settings called (no-op for Supabase)');
  }
};

// Firebase Auth wrapper untuk Supabase
const auth = {
...
// Firebase initializeApp wrapper
const firebase = {
  initializeApp: (config) => {
    console.log('Firebase initialized (Supabase wrapper)');
    return { auth, db };
  }
};

// Inisialisasi Firebase wrapper
...
async function openWAById(customerId) {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            const prospekDoc = await db.collection('prospek').doc(customerId).get();
                ...
                try {
                    ...
                    for (let row of json) {
                        ...
                        try {
                            ...
                            let formattedDeadline = getTodayDate();
                            ...
                            if (importType === 'transaksi') {
                                ...
                                await db.collection('db_transaksi').add({
                                    ...
                                    nama: nama || `Agent ${agentId}`,
                                    hp: cleanHp || '',
                                    upline_name: uplineName || '',
                                    upline_phone: uplinePhone || '',
                                    tanggal_transaksi: tanggalAkhir,
                                    status: 'pending_import',
                                    user_id: currentUser.uid,
                                    created_at: new Date().toISOString(),
                                    updated_at: new Date().toISOString()
