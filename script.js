...
document.getElementById('manageTargetBtn')?.addEventListener('click', () => { 
    if (currentUserRole === 'owner') {
        // Load current target values into modal
        document.getElementById('targetAgentInput').value = targetData.agent || 0;
        document.getElementById('targetKoorInput').value = targetData.koordinator || 0;
        document.getElementById('targetCAInput').value = targetData.ca || 0;
        document.getElementById('targetTransaksiInput').value = targetData.transaksi || 0;
...
document.getElementById('deadlineNotifBtn')?.addEventListener('click', async () => {
    try {
        ...
        let custQuery = supabase
            .from('customers')
            .select('nama, tanggal, agent_id, user_id')
            .lt('tanggal', today);
        let prosQuery = supabase
            .from('prospek')
            .select('nama, deadline, agent_type, user_id')
            .lt('deadline', today);
        ...
        if (total > 0) {
            let msg = `📅 DEADLINE TERLEWAT (${total} data):\n\n`;
            msg += `━━━━━━━━━━━━━━━━━━━━\n`;
            msg += `📞 CUSTOMER (${custOverdue?.length || 0}):\n`;
            custOverdue?.forEach(c => {
                msg += `  • ${c.nama} - ${c.agent_id || 'No ID'}\n`;
                msg += `    📅 Deadline: ${c.tanggal}\n`;
            });
            msg += `\n━━━━━━━━━━━━━━━━━━━━\n`;
            msg += `🎯 PROSPEK (${prosOverdue?.length || 0}):\n`;
            prosOverdue?.forEach(p => {
                msg += `  • ${p.nama} - ${p.agent_type || 'No Type'}\n`;
                msg += `    📅 Deadline: ${p.deadline}\n`;
            });
            msg += `\n━━━━━━━━━━━━━━━━━━━━\n`;
            msg += `⚠️ Segera lakukan followup pada data di atas!`;
            alert(msg);
        } else {
            showNotifTop('✅ Semua deadline terpenuhi!');
        }
        ...
        showNotifTop('❌ Gagal memeriksa deadline', true);
    }
});
...
document.getElementById('loginBtn')?.addEventListener('click', async function() {
    ...
    try {
        ...
        errorDiv.textContent = 'Login gagal: ' + e.message;
        this.textContent = originalText;
        this.disabled = false;
    }
});

// ========== ADDITIONAL EVENT LISTENERS FOR COMPLETENESS ==========

// ========== EDIT DEADLINE MODAL BUTTONS ==========
document.getElementById('saveDeadlineBtn')?.addEventListener('click', async () => {
    const newDeadline = document.getElementById('editDeadlineDate').value;
    if (!newDeadline) {
        showNotifTop('⚠️ Tanggal deadline harus diisi!', true);
...
document.getElementById('importAgentExcelBtn')?.addEventListener('click', () => {
    document.getElementById('agentExcelFile').click();
});

document.getElementById('exportAgentExcelBtn')?.addEventListener('click', exportAgentToExcel);

document.getElementById('importProdukExcelBtn')?.addEventListener('click', () => {
    document.getElementById('produkExcelFile').click();
});

document.getElementById('exportProdukExcelBtn')?.addEventListener('click', exportProdukToExcel);

document.getElementById('importTarifExcelBtn')?.addEventListener('click', () => {
    document.getElementById('tarifExcelFile').click();
});
...
document.getElementById('resetAgentFilterBtn')?.addEventListener('click', () => {
    document.getElementById('searchAgentInput').value = '';
    document.getElementById('filterUplineAgent').value = '';
    document.getElementById('filterCidAgent').value = '';
    document.getElementById('filterBankAgent').value = '';
    document.getElementById('filterDateAgent').value = '';
    document.getElementById('filterHasHpAgent').checked = false;
    document.getElementById('filterHasApkAgent').checked = false;
    renderAgentList(agentsData);
});

document.getElementById('resetTransaksiFilterBtn')?.addEventListener('click', async () => {
    document.getElementById('searchTransaksiInput').value = '';
    document.getElementById('filterProgresTransaksi').value = '';
    document.getElementById('filterStatusTransaksi').value = '';
    await loadDbTransaksi();
});

// ========== SELECT ALL BUTTONS FOR DATABASES ==========
document.getElementById('selectAllClosing')?.addEventListener('click', () => {
    ...
    checkboxes.forEach(cb => {
        cb.checked = !allChecked;
        const id = cb.dataset.id;
        if (!allChecked) selectedClosingIds.set(id, true);
...
document.getElementById('selectAllTidak')?.addEventListener('click', () => {
    ...
    checkboxes.forEach(cb => {
        cb.checked = !allChecked;
        const id = cb.dataset.id;
        if (!allChecked) selectedTidakIds.set(id, true);
...
document.getElementById('selectAllNomorSalah')?.addEventListener('click', () => {
    ...
    checkboxes.forEach(cb => {
        cb.checked = !allChecked;
        const id = cb.dataset.id;
        if (!allChecked) selectedNomorSalahIds.set(id, true);
...
document.getElementById('selectAllCommitment')?.addEventListener('click', () => {
    ...
    checkboxes.forEach(cb => {
        cb.checked = !allChecked;
        const id = cb.dataset.id;
        if (!allChecked) selectedCommitmentIds.set(id, true);
...
// ========== DELETE SELECTED BUTTONS FOR DATABASES ==========
document.getElementById('deleteSelectedClosing')?.addEventListener('click', async () => {
    const selectedIds = Array.from(selectedClosingIds.keys());
    if (selectedIds.length === 0) {
        showNotifTop('⚠️ Tidak ada data yang dipilih', true);
        return;
    }
    if (!confirm(`Hapus ${selectedIds.length} data closing?`)) return;
    for (const id of selectedIds) {
        await supabase.from('db_closing').delete().eq('id', id);
        selectedClosingIds.delete(id);
    }
    showNotifTop(`✅ ${selectedIds.length} data closing dihapus`);
    await loadDBClosing();
});

document.getElementById('deleteSelectedTidak')?.addEventListener('click', async () => {
    const selectedIds = Array.from(selectedTidakIds.keys());
    if (selectedIds.length === 0) {
        showNotifTop('⚠️ Tidak ada data yang dipilih', true);
        return;
    }
    if (!confirm(`Hapus ${selectedIds.length} data tidak tertarik?`)) return;
    for (const id of selectedIds) {
        await supabase.from('db_tidak_tertarik').delete().eq('id', id);
        selectedTidakIds.delete(id);
    }
    showNotifTop(`✅ ${selectedIds.length} data tidak tertarik dihapus`);
    await loadDBTidak();
});

document.getElementById('deleteSelectedNomorSalah')?.addEventListener('click', async () => {
    const selectedIds = Array.from(selectedNomorSalahIds.keys());
    if (selectedIds.length === 0) {
        showNotifTop('⚠️ Tidak ada data yang dipilih', true);
        return;
    }
    if (!confirm(`Hapus ${selectedIds.length} data nomor salah?`)) return;
    for (const id of selectedIds) {
        await supabase.from('nomor_salah').delete().eq('id', id);
        selectedNomorSalahIds.delete(id);
    }
    showNotifTop(`✅ ${selectedIds.length} data nomor salah dihapus`);
    await loadDBNomorSalah();
});

document.getElementById('deleteSelectedCommitment')?.addEventListener('click', async () => {
    const selectedIds = Array.from(selectedCommitmentIds.keys());
    if (selectedIds.length === 0) {
        showNotifTop('⚠️ Tidak ada data yang dipilih', true);
        return;
    }
    if (!confirm(`Hapus ${selectedIds.length} data commitment?`)) return;
    for (const id of selectedIds) {
        await supabase.from('db_commitment').delete().eq('id', id);
        selectedCommitmentIds.delete(id);
    }
    showNotifTop(`✅ ${selectedIds.length} data commitment dihapus`);
    await loadDBCommitment();
});

// ========== DELETE ALL BUTTONS FOR DATABASES ==========
document.getElementById('deleteAllClosing')?.addEventListener('click', async () => {
    if (!confirm('⚠️ PERINGATAN! Hapus SEMUA data closing?')) return;
    await supabase.from('db_closing').delete().neq('id', '');
    selectedClosingIds.clear();
    showNotifTop('✅ Semua data closing dihapus');
    await loadDBClosing();
});

document.getElementById('deleteAllTidak')?.addEventListener('click', async () => {
    if (!confirm('⚠️ PERINGATAN! Hapus SEMUA data tidak tertarik?')) return;
    await supabase.from('db_tidak_tertarik').delete().neq('id', '');
    selectedTidakIds.clear();
    showNotifTop('✅ Semua data tidak tertarik dihapus');
    await loadDBTidak();
});

document.getElementById('deleteAllNomorSalah')?.addEventListener('click', async () => {
    if (!confirm('⚠️ PERINGATAN! Hapus SEMUA data nomor salah?')) return;
    await supabase.from('nomor_salah').delete().neq('id', '');
    selectedNomorSalahIds.clear();
    showNotifTop('✅ Semua data nomor salah dihapus');
    await loadDBNomorSalah();
});

document.getElementById('deleteAllCommitment')?.addEventListener('click', async () => {
    if (!confirm('⚠️ PERINGATAN! Hapus SEMUA data commitment?')) return;
    await supabase.from('db_commitment').delete().neq('id', '');
    selectedCommitmentIds.clear();
    showNotifTop('✅ Semua data commitment dihapus');
    await loadDBCommitment();
});

// ========== TRANSACTION FILTERS ==========
document.getElementById('filterProgresTransaksi')?.addEventListener('change', () => {
    applyTransaksiFilters();
});

document.getElementById('filterStatusTransaksi')?.addEventListener('change', () => {
    applyTransaksiFilters();
});

document.getElementById('searchTransaksiInput')?.addEventListener('input', () => {
    clearTimeout(window.transaksiSearchTimeout);
    window.transaksiSearchTimeout = setTimeout(applyTransaksiFilters, 500);
});

async function applyTransaksiFilters() {
...
// ========== UPLINE BROADCAST SOURCE TYPE CHANGE ==========
document.querySelectorAll('input[name="uplineSourceType"]')?.forEach(radio => {
    radio.addEventListener('change', () => {
        const value = radio.value;
        const customerFilter = document.getElementById('uplineCustomerFilter');
        ...
        if (customerFilter) customerFilter.style.display = value === 'customer' ? 'flex' : 'none';
        if (transaksiFilter) transaksiFilter.style.display = value === 'transaksi' ? 'flex' : 'none';
        if (customCard) customCard.style.display = value === 'custom' ? 'block' : 'none';
...
supabase.auth.onAuthStateChange(async (event, session) => {
    ...
    if (session?.user) {
        ...
        if (error) { currentUserRole='cs'; currentUserName=currentUser.email?.split('@')[0]||'CS Agent'; currentUserEmail=currentUser.email||''; }
        else { currentUserRole=userData.role||'cs'; currentUserName=userData.nama||userData.email?.split('@')[0]||'CS Agent'; currentUserEmail=userData.email||''; const foto=userData.foto||'https://i.pravatar.cc/40'; document.getElementById('profileImg').src=foto; document.getElementById('previewFoto').src=foto; }
        ...
        const menuDbAgent = document.getElementById('menuDbAgent'), menuDbTransaksi = document.getElementById('menuDbTransaksi'), menuImport = document.getElementById('menuImport'), ownerMenu = document.getElementById('ownerMenu');
        if(currentUserRole==='owner') { if(menuDbAgent) menuDbAgent.style.display='flex'; if(menuDbTransaksi) menuDbTransaksi.style.display='flex'; if(menuImport) menuImport.style.display='flex'; if(ownerMenu) ownerMenu.style.display='block'; }
        else { if(menuDbAgent) menuDbAgent.style.display='none'; if(menuDbTransaksi) menuDbTransaksi.style.display='none'; if(menuImport) menuImport.style.display='none'; if(ownerMenu) ownerMenu.style.display='none'; }
